...
- Frontend: React + TypeScript + Vite client (`frontend/`) consuming trending, now-playing, paginated search, genre discovery, recommendations, movie details, similar-title endpoints, and shared release-year filtering across browse and recommendation flows.
...
- `GET /api/movies/trending?limit=...` - fetch today's trending movies (limit defaults to 10, max 20).
- `GET /api/movies/now-playing?limit=...` - fetch the current theatrical now-playing feed (limit defaults to 10, max 20).
