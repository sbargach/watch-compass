...
public class CachedMovieCatalogTests
{
    ...
    [Test]
    public async Task GetNowPlayingAsync_CachesNowPlayingResults()
    {
        var inner = new CountingMovieCatalog();
        var catalog = CreateCatalog(inner);

        await catalog.GetNowPlayingAsync();
        await catalog.GetNowPlayingAsync();

        inner.NowPlayingCalls.ShouldBe(1);
    }
    ...
    private sealed class CountingMovieCatalog : IMovieCatalog
    {
        ...
        public int NowPlayingCalls { get; private set; }
        ...
        {
            NowPlayingCalls++;
            return Task.FromResult<IReadOnlyList<MovieCard>>(new[] { new MovieCard(1000, "Now Playing", 105, Array.Empty<string>()) });
        }
    }
}
