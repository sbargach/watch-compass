...
public sealed class TmdbMovieCatalog : IMovieCatalog
{
    ...
    private static readonly KeyValuePair<string, object?> DiscoverTag = new("operation", "discover");
    ...
    private static readonly KeyValuePair<string, object?> NowPlayingTag = new("operation", "now-playing");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var genreLookup = await GetGenreLookupAsync(cancellationToken);
        var genreEntry = genreLookup.FirstOrDefault(entry =>
            entry.Key > 0 &&
            string.Equals(entry.Value, trimmedGenre, StringComparison.OrdinalIgnoreCase));
    ...
    public async Task<IReadOnlyList<MovieCard>> GetNowPlayingAsync(CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        EnsureApiKey();

        CallsCounter.Add(1, NowPlayingTag);
        try
        {
            var response = await _apiClient.GetNowPlayingMoviesAsync(cancellationToken);
            var genreLookup = response.Results.Any(result => result.GenreIds.Count > 0)
                ? await GetGenreLookupAsync(cancellationToken)
                : _genreLookup;
        ...
        {
            _logger.LogWarning(ex, "TMDB now playing movies failed.");
            throw;
        }
    }
    ...
    private async Task<PagedResult<MovieCard>> FetchPagedResultsAsync(
        int page,
        int pageSize,
        Func<int, CancellationToken, Task<TmdbSearchResponse>> fetchPage,
        KeyValuePair<string, object?> tag,
        CancellationToken cancellationToken)
    {
        CallsCounter.Add(1, tag);
        ...
        {
            var response = await fetchPage(tmdbPage, cancellationToken);
